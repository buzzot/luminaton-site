...
/**
 * Check whether an email is an admin.
 * Admin emails are taken from the ADMIN_EMAILS env var (comma-separated).
 * Falls back to MAIL_TO so the same address that receives the digests is also admin by default.
 */
function isAdmin(email) {
  if (!email) return false;
  const e = normalizeEmail(email);
  const sources = [process.env.ADMIN_EMAILS, process.env.MAIL_TO]
    .filter(Boolean)
    .join(',')
    .split(',')
    .map(s => normalizeEmail(s))
    .filter(Boolean);
  return sources.includes(e);
}

// Express middleware — require a valid session AND admin status (HTML)
function requireAdminPage(redirectTo = '/cabinet/dashboard') {
  return (req, res, next) => {
    const session = getSessionFromReq(req);
    if (!session) return res.redirect('/cabinet/');
    if (!isAdmin(session.email)) return res.redirect(redirectTo);
    req.session = session;
    next();
  };
}

// Express middleware — admin-only JSON API
function requireAdminApi() {
  return (req, res, next) => {
    const session = getSessionFromReq(req);
    if (!session) return res.status(401).json({ ok: false, error: 'Not signed in' });
    if (!isAdmin(session.email)) return res.status(403).json({ ok: false, error: 'Forbidden' });
    req.session = session;
    next();
  };
}
